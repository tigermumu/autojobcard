...
const AddWorkCardData: React.FC = () => {
  ...
  useEffect(() => {
    if (selectedIndex) {
      loadIndexData(Number(selectedIndex))
    }
  }, [selectedIndex])
  ...
  const handleOpenSaveModal = () => {
    ...
    // 尝试从选中的构型配置中获取默认值
    const selectedConfig = configurations.find(c => String(c.id) === selectedIndex)
    if (selectedConfig) {
      setSaveFormData({
        aircraft_number: '',
  ...
  const handleSaveToDatabase = async () => {
    ...
    try {
      ...
      const response = await workcardApi.saveCleanedData({
        ...
        configuration_id: Number(selectedIndex),
      ...
      if (response.success) {
        ...
        const errorDetails = response.errors && response.errors.length > 0 
          ...
          : ''
        message.error(`${errorMsg}${errorDetails}`, 10) // 显示10秒
  ...
  const handleExportExcel = () => {
    ...
    try {
      ...
      cleanedColumns.forEach(col => {
        ...
        } else if (col.title && typeof col.title === 'object') {
          // 处理复杂的title对象
  ...
  const handleDataCleanTest = async () => {
    ...
    try {
      ...
      if (response.success) {
        ...
        message.error('数据清洗失败: ' + (response.error || '未知错误'))
  ...
  return (
    <div style={{ padding: '24px', background: '#f0f2f5', minHeight: '100vh' }}>
      ...
      <Alert
        message="上传说明"
        description="请上传标准工卡数据的Excel文件，系统将自动解析并展示数据。上传后可以进行数据清洗和索引配置。"
        type="info"
        showIcon
          ...
          <Col span={8}>
            <Space>
              <Button 
                ...
                disabled={!selectedIndex || uploadedData.length === 0}
                loading={loading}
              >
                测试清洗(20条)
              </Button>
