...
echo [提示] 开始复制文件（使用 robocopy）...
echo.

REM 使用 robocopy 复制后端文件（排除 venv 和 __pycache__）
echo [1/4] 复制后端文件...
robocopy "%SOURCE_DIR%backend" "%TARGET_DIR%\backend" /E /XD venv __pycache__ .pytest_cache /XF *.pyc *.pyo *.pyd .Python /NFL /NDL /NJH /NJS
if errorlevel 8 (
    echo [错误] 复制失败
    pause
    exit /b 1
...
REM 使用 robocopy 复制前端文件（排除 node_modules）
echo [2/4] 复制前端文件...
robocopy "%SOURCE_DIR%frontend" "%TARGET_DIR%\frontend" /E /XD node_modules .vite dist /XF *.log .DS_Store /NFL /NDL /NJH /NJS
if errorlevel 8 (
    echo [错误] 复制失败
    pause
    exit /b 1
